...
1. 完成 [prerequisites](https://docs.iomesh.com/deploy/prerequisites) 章节中相关 open-iscsi 等前置配置
...
2. 部署 iomesh 集群 `iomesh-cluster-1`，配置 node 亲和性让其调度到 k8s-woker-{3~5}，另外需要注意 `iomesh-cluster-1.yaml` 中的注释部分，做如下配置
   ...
   a. 并将 chunk 和 redirector 中的 `dataCIDR` 字段配置为集群实际的存储网段
   
   b.将 `spec.chunk.devicemanager.blockDeviceNamespace` 配置为 iomesh-system (部署第一套 IOMesh 时同时部署了其他管控组件到 iomesh-system namesapce 中，所有的 blockDevice 均在 iomesh-system 中统一管理)
...
`kubectl apply -f iomesh-csi-configmap.yaml iomesh-cluster-1-csi-configmap.yaml `
...
| data.metaAddr      | iomesh meta service 地址，格式为 <iomesh-cluster-name>-client.<iomesh-cluster-namespace>.svc.cluster.local:10100 |



#### 为每个 IOMesh 集群创建分别对应的 StorageClass

`kubectl apply -f iomesh-sc.yaml iomesh-cluster-1-sc.yaml `
...
`kubectl apply -f iomesh-pvc.yaml iomesh-cluster1-pvc.yaml `
...
IOMesh 默认会开启拓扑感知功能，当 Pod 使用这些 PVC 时，会自动调度到对应 IOMesh 集群所在的  K8s Worker 上
